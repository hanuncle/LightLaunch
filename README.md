...
当前版本为 v0.1.1（MVP）。它采用原生 C++/Win32，不依赖 .NET、Qt、Electron、WebView 或后台服务。
...
- 右缘分组页使用 macOS Dock 风格的窄幅半透明圆角托盘，默认背景为 `RGB(244, 246, 244)`、透明度为 `53%`、边框为 `RGB(216, 221, 218)`；托盘高度随分组数量变化，每个小文件夹预览该组前 4 个目标图标
- 右键 Dock 空白区域可打开“Dock 外观设置”，独立修改 Dock 的背景颜色、背景透明度和边框线颜色
- Dock 右上角小图钉可独立锁住分组页；取消固定后恢复鼠标离开自动收回（固定状态仅本次运行有效）
- 新建、重命名和删除分组
- 按住分组图标拖到另一个分组位置，可调整 Dock 中的分组顺序并自动保存
- 所有新分组默认使用“3”分组确认过的浅色半透明玻璃围栏：背景 `RGB(230, 234, 231)`、透明度 `31%`、白色外描边；标题栏略实、内容区更通透，也可继续按分组单独调整
- 每个围栏都可拖动到桌面任意位置，并可从四边或四角自由调整大小；标题栏右上角提供独立图钉和关闭按钮
- 右键围栏空白区域打开“围栏背景设置”，可分别修改背景颜色、背景透明度和边框线颜色；设置按分组独立保存
...
3. 右键托盘空白处可新建分组，也可进入“Dock 外观设置”调整背景颜色、透明度和边框线颜色。
...
7. 在围栏内容区空白处右键添加应用、普通文件或文件夹，也可以进入“围栏背景设置”修改背景颜色、透明度和边框线颜色；直接把目标拖入即可添加，按住内容图标拖到另一个图标位置可调整内容顺序，双击内容图标启动，右键图标可重命名、移动或移除。
...
从 v0.1.1 开始，启动阶段会对短暂的文件占用进行有限重试。主配置不可读取但备份有效时，程序会明确提示并从备份加载；仅关闭程序不会自动覆盖主配置，首次修改时会再次确认。若主配置和备份都无法安全读取，程序会直接退出并保持两个文件不变，不再生成空白“常用”分组覆盖已有数据。

配置格式仍为 `SchemaVersion=2`，从 v0.1.0 升级无需迁移配置。
